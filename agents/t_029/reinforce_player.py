...
class myAgent():
    ...
    def getHeuristicValue(self, board):
        ...
        for i in range(11):
            for j in range(11):
                if j + 5 < 11:
                    horizontal = str(self.id) + "".join([str(board[(i, j+k)]) for k in range(5)])    
                    vertical = str(self.id) + "".join([str(board[(j+k, i)]) for k in range(5)])
                    v1, v2 = 0, 0
            ...
            for l in range(11):
                if l + 5 < 11 and i - 5 >= 0:
                    diagonal = str(self.id) + "".join([str(board[(i-k, l+k)]) for k in range(5)])
                    v3 = 0
        ...
        for i in range(11):
            for j in range(11):
                if j + 5 < 11:
                    ...
                    if horizontal.count(piece) > 3 or vertical.count(piece) > 3:
                        res += 1
                    count += 1
                else:
                    break
    ...
    def getFeatures(self, state, action):
        ...
        f1 = self.getHeuristicValue(next_state.board) / 11
